...
void Distributor::do_distribute_lesscost(int dis)
{
    ...
    // for(int i=2;i<site_names.size();i++){
    //     for(int j=2;j<customer_ids.size();j++){
    //         distirbute_table[i].line[j].demans.push_back(demands[dis][j].demans[0]);
    //         distirbute_table[i].line[j].demans.push_back(demands[dis][j].demans[1]);
    //     }
    // }
}
...
void Distributor::init_distribute_table()
{
    ...
    distirbute_table.resize(0);
        for (int i = 0; i < site_names.size(); i++)
        {
           Site_table temp_site_table;
            for (int j = 0; j < customer_ids.size(); j++)
            {
                Demans tmp_demans;
                temp_site_table.line.push_back(tmp_demans);
